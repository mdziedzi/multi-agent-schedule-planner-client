...
public class ReservationRequest {
    ...
    public ReservationRequest(Date bHour, Date eHour, int slotsNumber) throws negativeValueException {
        if (slots <= 0)
            throw new negativeValueException("Slots should be greater than zero: " + slots);
        slots = slotsNumber;
        beginHour = bHour;
        endHour = eHour;
    }
    ...
    public static ReservationRequest deserialize(String serializedReservationRequest) {
        ...
        try {
            final byte[] bytes = android.util.Base64.decode(serializedReservationRequest, android.util.Base64.DEFAULT);
